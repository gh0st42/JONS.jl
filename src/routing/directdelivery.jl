...
function directdelivery_add(sim::NetSim, routerId::Int16, message::Message)
  router = sim.nodes[routerId].router.core
  if length(router.store) < router.capacity
    push!(router.store, message)
    @process directdelivery_forward(sim.env, sim, routerId, message)
  else
    #println("Router full, dropping message")
  end
end

@resumable function directdelivery_forward(env::Environment, sim::NetSim, myId::Int16, message::Message)
...
function directdelivery_on_recv(env::Environment, sim::NetSim, src::Int16, myId::Int16, message::Message)
  router = sim.nodes[myId].router.core
  ...
  if length(router.store) < router.capacity
    if router_msg_known(sim, myId, message)
      ...
      if message.dst == myId
        ...
        @process directdelivery_forward(env, sim, myId, message)
      end
    end
  else
...
function directdelivery_init(sim::NetSim, node::Node)
end

function directdelivery_on_new_peer(env::Environment, sim::NetSim, mynodid::Int16, new_peer::Int16)
  router = sim.nodes[mynodid].router.core
  #println("PEERS: ", length(router.peers))
  #println("Router ", mynodid, " discovered new peer ", new_peer)
  for message in router.store
    @process directdelivery_forward(env, sim, mynodid, message)
  end
end

function DirectDeliveryRouter(capacity::Int, discovery_interval::Float64)::RouterImpl
  router = Router(capacity, discovery_interval)
  return RouterImpl("DirectDelivery", router, directdelivery_init, directdelivery_on_recv, directdelivery_on_new_peer, directdelivery_add)
end